...
Page({
    ...
    clickMemberAdd() {
        ...
        console.log("添加群聊", memberIds);
        wx.navigateTo({
            url: '/packageB/pages/qunLiao/qiaoLiao?type=1&groupId=' + that.data.groupId
                // encodeURIComponent：避免字符转换问题
                +
    ...
    clickMemberDelete() {
        ...
        wx.navigateTo({
            url: '/packageB/pages/qunLiao/qiaoLiao?type=2&groupId=' + that.data.groupId +
    ...
    clickGroupName() {
        ...
        // 跳转更改群名groupId确定群组name：有传递，没有不传递
        wx.navigateTo({
                url: '/packageB/pages/chatgroup/editName?groupId=' + that.data.groupId + name
            })
            /**
             * 修改群名分析
    ...
    sendPushMsg(groupType, time, timeTS) {
        ...
        db.collection('push_msg')
            ...
            .then(res => {
                console.log('发送推送消息成功 success', res);
            })
    },
    /**
     * 群聊成员的添加、删除、删除并退出
